...
final class WPBB_Form_Handler {
    ...
    private function verify_captcha_request() {
        ...
        if ($provider === 'hcaptcha') {
            ...
            $secret = sanitize_text_field(wpbb_get_option('hcaptcha_secret_key', ''));
            $token = isset($_POST['h-captcha-response']) ? sanitize_text_field(wp_unslash($_POST['h-captcha-response'])) : '';
            if (!$secret) return new WP_Error('wpbb_hcaptcha_missing_secret', __('hCaptcha secret key is missing in WP BBuilder settings.', 'wp-bbuilder'));
            if (!$token) return new WP_Error('wpbb_hcaptcha_missing_token', __('Please complete the hCaptcha challenge.', 'wp-bbuilder'));
            $response = wp_remote_post('https://hcaptcha.com/siteverify', ['timeout' => 12, 'body' => ['secret' => $secret, 'response' => $token, 'remoteip' => $_SERVER['REMOTE_ADDR'] ?? '']]);
            if (is_wp_error($response)) return $response;
            $body = json_decode(wp_remote_retrieve_body($response), true);
            if (empty($body['success'])) return new WP_Error('wpbb_hcaptcha_failed', __('hCaptcha verification failed. Please try again.', 'wp-bbuilder'));
            return true;
        }
