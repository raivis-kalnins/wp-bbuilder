(function (wp) {
  ...
  function sectionEdit(blockLabel, allowedBlocks) {
    return function (props) {
      var blockProps = useBlockProps({ className: 'wpbb-' + props.name.split('/')[1] });
      return el('div', blockProps,
        label(blockLabel),
        el(InnerBlocks, { allowedBlocks: allowedBlocks || undefined })
      );
    };
  }
  ...
  function colorInput(labelText, value, onChange, key) {
    return el('div', { key: key || labelText, style: { marginBottom: '12px' } }, [
      ...
      el('input', { key: 'c', type: 'color', value: value && /^#/.test(value) ? value : '#000000', onChange: function (e) { onChange(e.target.value); }, style: { width: '100%', height: '36px' } }),
      el(TextControl, { key: 't', label: 'Transparent / rgba / custom', value: value || '', onChange: onChange })
    ]);
  }

  function simpleTextEdit(blockLabel, fields) {
    return function (props) {
      ...
      return el(wp.element.Fragment, {},
        el(InspectorControls, {}, el(PanelBody, { title: 'Settings', initialOpen: true }, controls)),
        el('div', useBlockProps({ className: 'wpbb-' + props.name.split('/')[1] }),
          label(blockLabel),
          fields.length ? (props.attributes[fields[0].key] || blockLabel) : blockLabel
        )
      );
    };
  }
  ...
  registerBlockType('wpbb/row', {
    ...
    edit: function (props) {
      var className = 'wpbb-row row ' + (props.attributes.gutterX || '') + ' ' + (props.attributes.gutterY || '') + ' ' + (props.attributes.align ? ('justify-content-' + props.attributes.align) : '');
      var blockProps = useBlockProps({ className: className, style: { width: '100%', maxWidth: 'none' } });
      return el(wp.element.Fragment, {},
        el(InspectorControls, {},
          el(PanelBody, { title: 'Row settings', initialOpen: true }, [
            el(SelectControl, { key: 'containerClass', label: 'Bootstrap container', value: props.attributes.containerClass, options: [
              { label: 'None', value: '' },
            ...
            el(SelectControl, { key: 'gutterX', label: 'Horizontal gap', value: props.attributes.gutterX, options: [
              { label: 'gx-0', value: 'gx-0' }, { label: 'gx-1', value: 'gx-1' }, { label: 'gx-2', value: 'gx-2' }, { label: 'gx-3', value: 'gx-3' }
            ], onChange: function (v) { props.setAttributes({ gutterX: v }); } }),
            el(SelectControl, { key: 'gutterY', label: 'Vertical gap', value: props.attributes.gutterY, options: [
              { label: 'gy-0', value: 'gy-0' }, { label: 'gy-1', value: 'gy-1' }, { label: 'gy-2', value: 'gy-2' }, { label: 'gy-3', value: 'gy-3' }
            ], onChange: function (v) { props.setAttributes({ gutterY: v }); } }),
            el(SelectControl, { key: 'align', label: 'Alignment', value: props.attributes.align, options: [
              { label: 'Default', value: '' }, { label: 'Start', value: 'start' }, { label: 'Center', value: 'center' }, { label: 'End', value: 'end' }, { label: 'Between', value: 'between' }
            ], onChange: function (v) { props.setAttributes({ align: v }); } })
          ])
        ),
        el('div', blockProps,
          label('ROW'),
          props.attributes.containerClass ? el('div', { className: props.attributes.containerClass }, el(InnerBlocks, { allowedBlocks: ['wpbb/column'], orientation: 'horizontal' })) : el(InnerBlocks, { allowedBlocks: ['wpbb/column'], orientation: 'horizontal' })
        )
      );
    },
  ...
  registerBlockType('wpbb/button', {
    ...
    edit: function (props) {
      ...
      var style = { background: props.attributes.backgroundColor || undefined, color: props.attributes.textColor || undefined, borderColor: props.attributes.backgroundColor || undefined, display: 'inline-block', padding: '10px 14px', borderRadius: '12px' };
      return el(wp.element.Fragment, {},
        el(InspectorControls, {}, el(PanelBody, { title: 'Button settings', initialOpen: true }, [
          ...
          el(SelectControl, { key: 'variant', label: 'Variant', value: props.attributes.variant, options: [
            { label: 'Primary', value: 'primary' }, { label: 'Secondary', value: 'secondary' }, { label: 'Success', value: 'success' }, { label: 'Danger', value: 'danger' },
            { label: 'Warning', value: 'warning' }, { label: 'Info', value: 'info' }, { label: 'Light', value: 'light' }, { label: 'Dark', value: 'dark' }
          ], onChange: function (v) { props.setAttributes({ variant: v }); } }),
          el(SelectControl, { key: 'size', label: 'Size', value: props.attributes.size, options: [
            { label: 'Default', value: '' }, { label: 'Small', value: 'sm' }, { label: 'Large', value: 'lg' }
          ...
          el(TextControl, { key: 'backgroundColor', label: 'Background color', value: props.attributes.backgroundColor, onChange: function (v) { props.setAttributes({ backgroundColor: v }); } }),
          el(TextControl, { key: 'textColor', label: 'Text color', value: props.attributes.textColor, onChange: function (v) { props.setAttributes({ textColor: v }); } }),
  ...
  registerBlockType('wpbb/card', { title:'Card', icon:'id', category:'wpbb', edit:sectionEdit('CARD'), save:function(){ return el(InnerBlocks.Content); } });
  registerBlockType('wpbb/cards', { title:'Cards', icon:'grid-view', category:'wpbb', edit:sectionEdit('CARDS',['wpbb/card']), save:function(){ return el(InnerBlocks.Content); } });
  registerBlockType('wpbb/accordion', { title:'Accordion', icon:'menu', category:'wpbb', edit:sectionEdit('ACCORDION',['wpbb/accordion-item']), save:function(){ return el(InnerBlocks.Content); } });
  registerBlockType('wpbb/accordion-item', { title:'Accordion Item', icon:'excerpt-view', category:'wpbb', parent:['wpbb/accordion'], edit:sectionEdit('ACCORDION ITEM'), save:function(){ return el(InnerBlocks.Content); } });
  registerBlockType('wpbb/tabs', { title:'Tabs', icon:'index-card', category:'wpbb', edit:sectionEdit('TABS',['wpbb/tab-item']), save:function(){ return el(InnerBlocks.Content); } });
  registerBlockType('wpbb/tab-item', { title:'Tab Item', icon:'editor-table', category:'wpbb', parent:['wpbb/tabs'], edit:sectionEdit('TAB ITEM'), save:function(){ return el(InnerBlocks.Content); } });
  registerBlockType('wpbb/row-section', {
    title:'Row Section', icon:'cover-image', category:'wpbb',
    attributes:{ sectionClass:{type:'string',default:'py-5'}, containerClass:{type:'string',default:'container-fluid'}, backgroundClass:{type:'string',default:''} },
    edit:function(props){
      return el(wp.element.Fragment,{},
        el(InspectorControls,{},el(PanelBody,{title:'Row Section settings',initialOpen:true},[
          el(TextControl,{key:'sectionClass',label:'Section classes',value:props.attributes.sectionClass,onChange:function(v){props.setAttributes({sectionClass:v});}}),
        ...
        el('section', useBlockProps({className:'wpbb-row-section ' + (props.attributes.sectionClass || '') + ' ' + (props.attributes.backgroundClass || '')}),
          label('ROW SECTION'),
  ...
  registerBlockType('wpbb/table', {
    title:'Bootstrap Table', icon:'table-col-after', category:'wpbb',
    attributes:{ csvText:{type:'string',default:'Name,Role\nJohn,Designer\nAnna,Developer'}, csvFileName:{type:'string',default:''}, delimiter:{type:'string',default:','}, useFirstRowHeader:{type:'boolean',default:true}, tableClass:{type:'string',default:'table table-striped table-hover'}, responsive:{type:'boolean',default:true}, small:{type:'boolean',default:false}, bordered:{type:'boolean',default:false} },
    edit:function(props){
  ...
  registerBlockType('wpbb/dynamic-form', { title:'Dynamic Form', icon:'feedback', category:'wpbb', edit:sectionEdit('DYNAMIC FORM'), save:function(){ return null; } });
  registerBlockType('wpbb/cta-card', { title:'CTA Card', icon:'megaphone', category:'wpbb', attributes:{title:{type:'string',default:'CTA Card'}}, edit:simpleTextEdit('CTA CARD',[{key:'title',label:'Title'}]), save:function(){ return null; } });
  registerBlockType('wpbb/cta-section', { title:'CTA Section', icon:'cover-image', category:'wpbb', attributes:{title:{type:'string',default:'CTA Section'}}, edit:simpleTextEdit('CTA SECTION',[{key:'title',label:'Title'}]), save:function(){ return null; } });
  registerBlockType('wpbb/google-map', { title:'Google Map', icon:'location-alt', category:'wpbb', attributes:{embedUrl:{type:'string',default:''},height:{type:'string',default:'380px'}}, edit:simpleTextEdit('GOOGLE MAP',[{key:'embedUrl',label:'Embed URL'},{key:'height',label:'Height'}]), save:function(){ return null; } });
  registerBlockType('wpbb/menu-option', { title:'Menu Option', icon:'menu', category:'wpbb', attributes:{title:{type:'string',default:'Menu Item'},price:{type:'string',default:''}}, edit:simpleTextEdit('MENU OPTION',[{key:'title',label:'Title'},{key:'price',label:'Price'}]), save:function(){ return null; } });
  registerBlockType('wpbb/sitemap', { title:'Sitemap', icon:'networking', category:'wpbb', attributes:{title:{type:'string',default:'Sitemap'}}, edit:simpleTextEdit('SITEMAP',[{key:'title',label:'Title'}]), save:function(){ return null; } });
  registerBlockType('wpbb/soc-follow-block', { title:'Social Follow', icon:'share', category:'wpbb', attributes:{title:{type:'string',default:'Follow Us'}}, edit:simpleTextEdit('SOC FOLLOW',[{key:'title',label:'Title'}]), save:function(){ return null; } });
  registerBlockType('wpbb/soc-share', { title:'Social Share', icon:'share-alt2', category:'wpbb', attributes:{title:{type:'string',default:'Share'}}, edit:simpleTextEdit('SOC SHARE',[{key:'title',label:'Title'}]), save:function(){ return null; } });
  registerBlockType('wpbb/video', { title:'Video', icon:'format-video', category:'wpbb', attributes:{videoUrl:{type:'string',default:''}}, edit:simpleTextEdit('VIDEO',[{key:'videoUrl',label:'Video URL'}]), save:function(){ return null; } });
  ...
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title: 'CTA Card settings', initialOpen: true }, [
        ...
        el(TextControl, { key:'bgColor', label:'Background color', value: props.attributes.bgColor, onChange:function(v){ props.setAttributes({bgColor:v}); } }),
        el(TextControl, { key:'textColor', label:'Text color', value: props.attributes.textColor, onChange:function(v){ props.setAttributes({textColor:v}); } })
      ])),
      el('div', useBlockProps({ className:'wpbb-cta-card card h-100', style:{ background: props.attributes.bgColor || undefined, color: props.attributes.textColor || undefined } }), label('CTA CARD'), el('div', { className:'card-body' }, [el('strong', { key:'t' }, props.attributes.title), el('p', { key:'p' }, props.attributes.text)]))
    );
  },
  save: function(){ return null; }
});
...
registerBlockType('wpbb/cta-section', {
  ...
  attributes: { title:{type:'string',default:'CTA Section'}, text:{type:'string',default:'Call to action text'}, buttonText:{type:'string',default:'Get started'}, buttonUrl:{type:'string',default:'#'}, bgColor:{type:'string',default:''}, textColor:{type:'string',default:''} },
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title: 'CTA Section settings', initialOpen: true }, [
        el(TextControl, { key:'title', label:'Title', value: props.attributes.title, onChange:function(v){ props.setAttributes({title:v}); } }),
        el(TextareaControl, { key:'text', label:'Text', value: props.attributes.text, onChange:function(v){ props.setAttributes({text:v}); } }),
        el(TextControl, { key:'buttonText', label:'Button text', value: props.attributes.buttonText, onChange:function(v){ props.setAttributes({buttonText:v}); } }),
        el(TextControl, { key:'buttonUrl', label:'Button URL', value: props.attributes.buttonUrl, onChange:function(v){ props.setAttributes({buttonUrl:v}); } }),
      ...
      el('section', useBlockProps({ className:'wpbb-cta-section text-center py-5', style:{ background: props.attributes.bgColor || undefined, color: props.attributes.textColor || undefined } }), label('CTA SECTION'), el('h3', {}, props.attributes.title), el('p', {}, props.attributes.text))
    );
  },
  save: function(){ return null; }
});
...
registerBlockType('wpbb/google-map', {
  ...
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title: 'Google Map settings', initialOpen: true }, [
...
registerBlockType('wpbb/menu-option', {
  ...
  attributes: { title:{type:'string',default:'Menu Item'}, price:{type:'string',default:''}, text:{type:'string',default:''}, bgColor:{type:'string',default:''}, textColor:{type:'string',default:''} },
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title: 'Menu Option settings', initialOpen: true }, [
        el(TextControl, { key:'title', label:'Title', value: props.attributes.title, onChange:function(v){ props.setAttributes({title:v}); } }),
        el(TextControl, { key:'price', label:'Price', value: props.attributes.price, onChange:function(v){ props.setAttributes({price:v}); } }),
        el(TextareaControl, { key:'text', label:'Text', value: props.attributes.text, onChange:function(v){ props.setAttributes({text:v}); } }),
        el(TextControl, { key:'bgColor', label:'Background color', value: props.attributes.bgColor, onChange:function(v){ props.setAttributes({bgColor:v}); } }),
        el(TextControl, { key:'textColor', label:'Text color', value: props.attributes.textColor, onChange:function(v){ props.setAttributes({textColor:v}); } })
      ])),
      el('div', useBlockProps({ className:'wpbb-menu-option', style:{ background: props.attributes.bgColor || undefined, color: props.attributes.textColor || undefined } }), label('MENU OPTION'), props.attributes.title + ' ' + props.attributes.price)
    );
  },
  save: function(){ return null; }
});
...
registerBlockType('wpbb/sitemap', {
  ...
  attributes: { title:{type:'string',default:'Sitemap'}, showPages:{type:'boolean',default:true} },
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title: 'Sitemap settings', initialOpen: true }, [
        el(TextControl, { key:'title', label:'Title', value: props.attributes.title, onChange:function(v){ props.setAttributes({title:v}); } }),
      ...
      el('div', useBlockProps({ className:'wpbb-sitemap' }), label('SITEMAP'), props.attributes.title)
    );
  },
  save: function(){ return null; }
});
...
registerBlockType('wpbb/soc-follow-block', {
  ...
  attributes: { title:{type:'string',default:'Follow Us'} },
  edit: function(props){
    return el('div', useBlockProps({ className:'wpbb-soc-follow' }), label('SOC FOLLOW'), props.attributes.title + '  FB IG LI');
  },
  save: function(){ return null; }
});
...
registerBlockType('wpbb/soc-share', {
  ...
  attributes: { title:{type:'string',default:'Share'} },
  edit: function(props){
    return el('div', useBlockProps({ className:'wpbb-soc-share' }), label('SOC SHARE'), props.attributes.title + '  FB X LI');
  },
  save: function(){ return null; }
});
...
registerBlockType('wpbb/video', {
  ...
  attributes: { videoUrl:{type:'string',default:''}, ratioClass:{type:'string',default:'ratio ratio-16x9'} },
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title:'Video settings', initialOpen:true }, [
...
registerBlockType('wpbb/whatsapp-chat', {
  title: 'WhatsApp Chat', icon: 'whatsapp', category: 'wpbb',
  attributes: { label:{type:'string',default:'Chat on WhatsApp'}, phone:{type:'string',default:''}, message:{type:'string',default:''}, position:{type:'string',default:''}, bgColor:{type:'string',default:''}, textColor:{type:'string',default:''} },
  edit: function(props){
    return el(wp.element.Fragment, {},
      el(InspectorControls, {}, el(PanelBody, { title:'WhatsApp Chat settings', initialOpen:true }, [
        el(TextControl, { key:'label', label:'Label', value: props.attributes.label, onChange:function(v){ props.setAttributes({label:v}); } }),
        el(TextControl, { key:'phone', label:'Phone', value: props.attributes.phone, onChange:function(v){ props.setAttributes({phone:v}); } }),
        el(TextControl, { key:'message', label:'Message', value: props.attributes.message, onChange:function(v){ props.setAttributes({message:v}); } }),
        el(SelectControl, { key:'position', label:'Position', value: props.attributes.position, options:[{label:'Admin default',value:''},{label:'bottom-right',value:'bottom-right'},{label:'bottom-left',value:'bottom-left'},{label:'top-right',value:'top-right'},{label:'top-left',value:'top-left'}], onChange:function(v){ props.setAttributes({position:v}); } }),
        el(TextControl, { key:'bgColor', label:'Background color', value: props.attributes.bgColor, onChange:function(v){ props.setAttributes({bgColor:v}); } }),
        el(TextControl, { key:'textColor', label:'Text color', value: props.attributes.textColor, onChange:function(v){ props.setAttributes({textColor:v}); } })
      ])),
      el('div', useBlockProps({ className:'wpbb-whatsapp-chat' }), label('WHATSAPP CHAT'), el('div', { style:{ display:'inline-block', background: props.attributes.bgColor || '#25D366', color: props.attributes.textColor || '#fff', padding:'12px 16px', borderRadius:'999px' } }, props.attributes.label))
    );
  },
  save: function(){ return null; }
});
registerBlockType('wpbb/dynamic-form', {
  ...
  icon:'feedback',
  category:'wpbb',
  attributes:{ formTitle:{type:'string',default:'Contact form'}, recipient:{type:'string',default:''}, emailSubject:{type:'string',default:'New form submission'}, successMessage:{type:'string',default:'Thank you for your submission!'}, submitText:{type:'string',default:'Submit'}, fieldsJson:{type:'string',default:''} },
  edit:function(props){
    var fields = [];
    ...
    function updateField(index, key, value){ var next = fields.slice(); next[index] = Object.assign({}, next[index], (function(){ var o={}; o[key]=value; return o; })()); saveFields(next); }
    function addField(){ var next = fields.slice(); next.push({ type:'text', name:'field_' + (fields.length + 1), label:'New field', required:false, width:6, placeholder:'', options:'' }); saveFields(next); }
    return el(wp.element.Fragment, {},
      ...
      })).concat([el('button', { key:'add', type:'button', className:'components-button is-secondary', onClick:addField }, 'Add field')]))),
      el('div', useBlockProps({ className:'wpbb-dynamic-form-editor' }), label('DYNAMIC FORM'),
        el('strong', {}, props.attributes.formTitle),
        el('div', { className:'wpbb-form-preview-grid' }, fields.map(function(field, index){
